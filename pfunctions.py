...
def pcshutdown():
    # 关闭计算机
    if shutdown_enabled:
        session = winrm.Session(device_ip, auth=(pc_account, pc_password))
        _ = session.run_cmd(f'shutdown -s -t {shutdown_time}')
...
def send_message(content):
    if push_enabled:
        ...
        if push_bark_enabled:
            try:
                bark = BarkNotificator(device_token=push_bark_token)
                bark.send(title=device_name + content, content=get_time(),ringtone="1")
            except Exception as e:
                p_print("Bark发送消息出错：" + str(e))
...
if push_enabled:
    ...
    push_bark_enabled = checkbool(yd['functions']['push_notifications']['Bark']['enabled'])
    if push_bark_enabled:
        push_bark_token = trans_str(yd['functions']['push_notifications']['Bark']['token'])

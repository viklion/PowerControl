...
class PowerControl():
    def __init__(self):
        ...
        if self.enabled:
            ...
            self.wol_enabled = get_var('wol_enabled')
            self.shutdown_enabled = get_var('shutdown_enabled')
    ...
    def check_state(self, is_power_off):
        if self.ping_enabled:
            ...
            if self.pc_state != new_state:
                ...
                substr = f'cmd=2&uid={self.uid}&topic={self.topic}/up&msg={update_state}\r\n'
                self.tcp_client_socket.send(substr.encode("utf-8"))
        ...
        if state == 'on' :
            ...
            if self.shutdown_enabled:
                try:
                    ...
                    print_and_log("关机指令发送失败："+ str(e) ,3)
